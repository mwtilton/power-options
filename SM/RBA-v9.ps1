...
$getExportFolder = "X:\IT\NOC\Biometric Audit Reports\Exports\*"
...
If($tpExportsFolder -eq $True)
{
    ...
    $getClientFolder = Get-ChildItem "X:\IT\NOC\Biometric Audit Reports\$custID*"
    $getClientFolderName = Get-ChildItem "X:\IT\NOC\Biometric Audit Reports\$custID*" | % {$_.basename}
    ...
    If($tpClientCSVArchiveFolder -eq $True)
    {
        ...
        $getBioReportCSVbasename = Get-ChildItem "X:\IT\NOC\Biometric Audit Reports\Exports\*.csv" | % { $_.basename }
        ...
        foreach($file in $getBioReportCSVbasename)
        {
            ...
            Get-Content "X:\IT\NOC\Biometric Audit Reports\Exports\$file.csv" -Encoding Default | select -Skip 1 | ConvertFrom-Csv -UseCulture -Header $newHeaders | Export-Csv -Path $outChangeHeaders -Append -NoTypeInformation -Force
            ...
            #Move-Item -Path "X:\IT\NOC\Biometric Audit Reports\Exports\$file.csv" -Destination $getClientFolder\CSVArchive\$year\$dateFormat3 -ErrorAction SilentlyContinue
        }
